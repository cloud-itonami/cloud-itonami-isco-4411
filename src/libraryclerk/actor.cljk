(ns libraryclerk.actor
  "LibraryClerkActor — the ISCO-08 4411 independent library & archive
  support practice actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  circulation-desk operation request (intake → advise → govern →
  ...
  The unconditional invariant: the LibraryCirculationAdvisor can never
  directly commit a record or dispatch the shelving/sorting/retrieval
  robot the LibraryServicesGovernor refuses — every commit-record!
  call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [libraryclerk.advisor :as advisor]
            [libraryclerk.governor :as governor]
            [libraryclerk.store :as store]))

(defn build-graph
  "Build a compiled LibraryClerkActor graph. `store` implements
  `libraryclerk.store/Store`. `advisor` implements
  `libraryclerk.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:patron-id (:patron-id request)
                                    :op (:op proposal)
